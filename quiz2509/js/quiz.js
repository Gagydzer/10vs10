...
window.onload=function(){
	...
	var vm = new Vue({ 
		...
		computed: {
			...
			currentBgCategoryId: function() {
				try {
					return this.asks[this.currentask].config.categoryId
				} catch (error) {
					return 0
				}	
			},
		...
		created: function() {
			var self = this;
						function compStyle(currentValue) {
							...
							if (!url)  {
								this.bgCategories.findIndex((element) => {if (id == element.id) {							
